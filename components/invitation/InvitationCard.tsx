...
import { FileHeart, Eye, Calendar, Edit, Share2, Trash2 } from "lucide-react";
...
const statusConfig = {
  DRAFT: { label: "임시저장", color: "bg-stone-100 text-stone-700" },
  PUBLISHED: { label: "게시됨", color: "bg-green-100 text-green-700" },
  EXPIRED: { label: "만료됨", color: "bg-red-100 text-red-700" },
  DELETED: { label: "삭제됨", color: "bg-stone-100 text-stone-400" },
};
...
export function InvitationCard({
  ...
  const handleDeleteClick = async () => {
    const confirmed = await confirm({
      title: "청첩장을 삭제하시겠습니까?",
      description: `${groomName} & ${brideName}의 청첩장이 영구적으로 삭제됩니다. 이 작업은 되돌릴 수 없습니다.`,
      confirmText: "삭제",
      cancelText: "취소",
      variant: "danger",
    });
  ...
  return (
    <div className="group relative bg-white rounded-lg border border-stone-200 overflow-hidden hover:border-stone-300 transition-colors">
      {/* Thumbnail */}
      <div className="relative aspect-[4/3] bg-stone-100 overflow-hidden">
        {thumbnailUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <FileHeart className="w-16 h-16 text-stone-300" />
          </div>
        )}
        ...
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-300 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
          <button
            onClick={handleEdit}
            className="p-3 bg-white rounded-full hover:bg-stone-50 transition-colors shadow-lg"
            title="편집"
          >
      ...
      <div className="p-5">
        {/* Title */}
        <h3 className="text-base font-medium text-stone-900 mb-2 truncate">
          {groomName} & {brideName}
        </h3>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-stone-100">
          <span className="text-xs text-stone-500">{relativeDate}</span>
          <Link
            href={`/editor/${id}`}
            target="_blank"
